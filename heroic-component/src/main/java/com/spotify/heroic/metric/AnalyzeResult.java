...
public final class AnalyzeResult {
    ...
    public AnalyzeResult(
        ...
        @JsonProperty("data") List<AggregationData> data, @JsonProperty("cadence") Duration cadence,
    ...
    public static Collector<AnalyzeResult, AnalyzeResult> collect(final Identifier what) {
        ...
        return results -> {
            Duration cadence = Duration.empty();
            ...
            return new AnalyzeResult(AnalyzeResult.IN, AnalyzeResult.OUT, data.build(), cadence,
                nodes.build(), edges.build(), errors.build(), tracer.end(traces.build()));
        };
    }
    ...
    public static AnalyzeResult analyze(
        ...
        final Queue<AggregationContext> queue = new LinkedList<>();
        queue.add(out);
        nodes.put(out.id(), new AnalyzeResult.AnalyzeNode(out.name().orElse(IN)));
        ...
        nodes.put(OUT, new AnalyzeResult.AnalyzeNode(OUT));
        edges.add(new AnalyzeResult.AnalyzeEdge(out.id(), OUT, initial));

        while (!queue.isEmpty()) {
            final AggregationContext node = queue.poll();

            for (final AggregationContext p : node.parents()) {
                final List<Map<String, String>> keys =
                    p.input().stream().map(s -> s.getKey()).collect(Collectors.toList());

                edges.add(new AnalyzeResult.AnalyzeEdge(p.id(), node.id(), keys));
                ...
                queue.add(p);
                nodes.put(p.id(), new AnalyzeResult.AnalyzeNode(p.name().orElse(IN)));
            }
        }
        ...
        return new AnalyzeResult(AnalyzeResult.IN, AnalyzeResult.OUT, data.build(), out.cadence(),
            nodes.build(), edges.build(), errors.build(), trace.get());
    }
    ...
    public static Transform<Throwable, AnalyzeResult> nodeError(
        ...
        return (Throwable e) -> {
            ...
            return new AnalyzeResult("", "", EMPTY_DATA, Duration.empty(), EMPTY_NODES, EMPTY_EDGES,
