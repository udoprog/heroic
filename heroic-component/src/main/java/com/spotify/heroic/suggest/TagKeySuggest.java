...
public class TagKeySuggest {
    ...
    @JsonCreator
    public TagKeySuggest(@JsonProperty("errors") List<RequestError> errors,
    ...
    public TagKeySuggest(List<String> values, boolean limited) {
        this(EMPTY_ERRORS, values, limited);
    }

    public static Collector<TagKeySuggest, TagKeySuggest> reduce(final int limit) {
        return new Collector<TagKeySuggest, TagKeySuggest>() {
            @Override
            public TagKeySuggest collect(Collection<TagKeySuggest> groups) throws Exception {
                final List<RequestError> errors = new ArrayList<>();
                final List<String> values = new ArrayList<>();

                boolean limited = false;

                for (final TagKeySuggest g : groups) {
                ...
                return new TagKeySuggest(errors,
                        values.subList(0, Math.min(values.size(), limit)), limited);
            }
        };
    }

    public static Transform<Throwable, ? extends TagKeySuggest> nodeError(
            final NodeRegistryEntry node) {
        return new Transform<Throwable, TagKeySuggest>() {
            @Override
            public TagKeySuggest transform(Throwable e) throws Exception {
                final NodeMetadata m = node.getMetadata();
                final ClusterNode c = node.getClusterNode();
                return new TagKeySuggest(
    ...
    public static Transform<Throwable, ? extends TagKeySuggest> nodeError(
        ...
        return new Transform<Throwable, TagKeySuggest>() {
            ...
            public TagKeySuggest transform(Throwable e) throws Exception {
                ...
                return new TagKeySuggest(errors, EMPTY_VALUES, false);
            }
        };
    }

    public static TagKeySuggest empty() {
        return new TagKeySuggest(EMPTY_ERRORS, EMPTY_VALUES, false);
    }
}
