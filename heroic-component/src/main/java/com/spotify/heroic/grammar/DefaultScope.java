...
public class DefaultScope implements Expression.Scope {
    ...
    private DefaultScope(final Map<String, Expression> scope) {
        this.scope = scope;
    }
    ...
    private Map<String, Expression> buildScope(final long now) {
        ...
        scope.put("now", Expression.integer(now));
        return scope.build();
    }
    ...
    @Override
    public Expression.Scope adjusted(
        final Map<String, Expression> added
    ) {
        final ImmutableMap.Builder<String, Expression> scope = ImmutableMap.builder();
        scope.putAll(this.scope).putAll(added);
        return new DefaultScope(scope.build());
    }
}
